...
* **Stale listings now get retired, daily (2026-07-28).** Johannes asked whether jobs that can no longer be applied to are kept out of the daily runs. They were not: `scripts/ingest-jobs.ts` only ever set `is_active: true` and nothing anywhere set it false, so dead listings accumulated forever. Measured before fixing: **78 of 394 active jobs (20%) were not in that day's feeds** yet stayed live on the site.
  * **Feed absence is deliberately not treated as proof a job is gone.** RemoteOK's API returns its most recent 100 while we held 127 active RemoteOK jobs, so at least 27 fall out of the window every day purely by ageing. Retiring on "missing from today's feed" would kill live listings.
  * **Link checking was tested per source against real dead rows rather than assumed.** Only **Remotive** gives a usable answer (a true 404). **RemoteOK and Arbeitnow answer 200 for closed and open listings alike**, and **Himalayas answers 403 to any request from us**. So `LINK_CHECKABLE_SOURCES` in `lib/utils/job-freshness.ts` contains remotive alone, and a source should only be added there after checking a real dead listing from it.
  * **Age is the primary rule.** `MAX_JOB_AGE_DAYS = 40`, chosen by Johannes. A job with no posting date is left alone rather than guessed into expiry, and a network timeout during a link check is never read as "gone", so the job gets another chance the next day.
  * Retirement is reversible: `is_active` goes false, the row stays.
  * **`validThrough` added to the JobPosting JSON-LD** on `app/jobs/[slug]/page.tsx`, set to posting date plus `MAX_JOB_AGE_DAYS`. Without it Google had no way to know when we consider a listing expired, and serving JobPosting markup for filled roles is exactly what earns a structured data penalty. Same family of problem as the non-job listings cleaned up earlier.
  * Verified by running the real ingestion: 0 retired by age (nothing was over 40 days yet), **1 retired by link check**, and that job was independently re-checked afterwards and does return 404. `validThrough` confirmed in the live page source (posted 2026-07-27, valid through 2026-09-05). The daily `daily-job-ingestion` scheduled task picks all of this up with no change needed, its output now carries an extra "Freshness:" line.
