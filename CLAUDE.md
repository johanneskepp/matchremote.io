...
**Stripe Test mode (2026-08-02):** `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`,
`STRIPE_WEBHOOK_SECRET` are set in both `.env.local` and Vercel Production
(Test mode values, safe there since they only touch Stripe's Test ledger).
Fully verified working, see the Stripe Working entry. Live keys not
requested yet, see "Still blocked" below.
...
4. ~~Stripe account, Test mode product, checkout verification~~ **Done (2026-08-02).** Fully built and verified in Test mode, see the "Stripe Test mode built out end to end" Working entry below. Only remaining step is flipping to Live keys, which needs Johannes's explicit go ahead first, same as Paddle's Sandbox to Live flip. Do not re-ask about the Test mode setup itself.
...
* **Stripe Test mode built out end to end and fully verified the same day (2026-08-02).** Johannes created the Stripe account and pasted the Test mode secret key in chat. From there Claude Code drove the rest directly through the Stripe API rather than the dashboard: created the `matchremote` product and its $7/week price (`stripe.products.create` / `stripe.prices.create`), created a webhook endpoint pointed at `https://matchremote.io/api/webhooks/stripe` listening for `customer.subscription.*` (`stripe.webhookEndpoints.create`, which also returns the signing secret directly, no dashboard copy paste needed), and wrote all three values into `.env.local` and Vercel Production (Test mode keys, safe to have live since they only touch Stripe's Test ledger, exactly the same reasoning as testing the Paddle Sandbox build against the real webhook endpoint).
  ...
  * **Not done yet, on purpose**: no Live keys requested or used, this is Test mode start to finish, matching the same phased discipline the Paddle build used. Flip to Live only after Johannes explicitly asks for it, same as before.
